surfaceForward <- function(otree, 
                           odata, 
                           starting_list=NULL, 
                           starting_shifts=NULL, 
                           exclude=0, 
                           aic_threshold=0, 
                           max_steps=NULL, 
                           save_steps=FALSE, 
                           filename="temp_out_list.R", 
                           verbose=FALSE, 
                           plotaic=FALSE, 
                           error_skip=FALSE, 
                           sample_shifts=FALSE, 
  ...
  for (j in (length(out_list)+1):max_steps) {
    out_list[[j]] <- addRegime(otree,
                               odata,
                               oldshifts=out_list[[j-1]]$savedshifts,
                               oldfit=out_list[[j-1]]$fit,
                               oldaic=out_list[[j-1]]$aic, 
                               alloldaic=out_list[[j-1]]$all_aic,
                               exclude=exclude, 
