collapseRegimes <- function(otree, 
                            odata, 
                            oldshifts, 
                            oldaic, 
                            oldfit, 
  ...
  k <- length(oldshifts)
  kk <- length(uniqueshifts)
  npair <- (kk*(kk-1))/2
  # aics <- data.frame(X = rep(NA,npair), Y=rep(NA,npair), aic=rep(NA,npair), daic=rep(NA,npair))
  if (verbose) {
    print(paste(kk, "unique regimes: testing", npair,"pairwise collapse(s)"),quote=F)
  }
  Comb <- combn(kk, 2)
  if (ncores == 1) {
    aics <- foreach(y = 1:ncol(Comb),
                    .combine = 'rbind',
                    .packages = c('ouch')) %do% fitHansenBackward(j = Comb[1, y], 
                                                                  i = Comb[2, y], 
                                                                  k, 
                                                                  n,
                                                                  kk, 
    ...
    if (only_best==TRUE | dim(df)[1]==1) {
      ...
      if (sample_shifts&df$daic[z] < (aic_threshold)) {
        ...
        if (length(candidates) > 1) {
          z<-sample(candidates, 1)
        }
      }
      if ((dim(df)[1] == 1) & verbose) {
        print("1 cluster(s):", quote=F)
        print(2,quote=F)
      }
      if (verbose) {
        print("collapsing:",quote=F)
        print(as.character(df[z,1:2]),quote=F)
      }
      ...
      if (verbose) {
        print(paste(cl$no,"cluster(s):"), quote=F)
        print(cl$csize, quote=F)
      }
      for (m in 1:cl$no) {
  ...
  if (plotaic) {
    plot(aics$aic, ylim = c(min(best_aic,oldaic), oldaic+20), xlim=c(0,dim(aics)[1]))
    abline(h=oldaic-seq(0,30, by=2))
    abline(h=best_aic, col="red")
  }
...
fitHansenBackward <- function(j, 
  ...
  Out <- data.frame(X = as.character(uniqueshifts[i]), 
                    Y = as.character(uniqueshifts[j]), 
                    aic = tempaic, 
                    daic = tempaic - oldaic)
  return(Out)
}
