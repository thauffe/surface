surfaceBackward <- function(otree,
                            odata,
                            starting_model,
                            aic_threshold = 0, 
                            max_steps =  NULL,
                            save_steps = FALSE,
                            filename = "temp_back_list.R",
                            verbose = FALSE, 
                            only_best = FALSE, 
                            plotaic = FALSE, 
                            error_skip = FALSE, 
  ...
  back_list <- list()
  back_list[[1]] <- list(fit=starting_model$fit,
                         delta_aic=NULL, 
                         aic=starting_model$aic,
                         savedshifts=starting_model$savedshifts,
                         n_regimes=starting_model$n_regimes)
  for (j in 2:max_steps) {
    back_list[[j]] <- collapseRegimes(otree,
                                      odata,
                                      oldshifts=back_list[[j-1]]$savedshifts,
                                      oldaic=back_list[[j-1]]$aic,
                                      oldfit=back_list[[j-1]]$fit, 
